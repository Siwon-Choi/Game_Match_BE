...
현재 프로젝트는 세 가지 DB 프로필을 지원합니다.

| Profile | Database | Purpose |
| --- | --- | --- |
| `local` | H2 file DB | 빠른 로컬 실행 |
| `mysql` | MySQL | MySQL 기반 개발 환경 |
| `supabase` | PostgreSQL | Supabase 배포/운영형 데이터베이스 |

Supabase profile에서는 PostgreSQL의 대소문자 식별자 문제를 피하기 위해 Hibernate quoted identifier 설정을 사용합니다.
...
JWT_REFRESH_TOKEN_EXPIRATION_MS=31536000000
```

Supabase를 사용할 경우 다음 값을 추가합니다.
...
### 3. Seed Supabase data

Supabase 연결값을 `.env`에 넣은 뒤 seed script를 실행합니다.

```bash
python3 scripts/seed_supabase.py
```

Seed script는 내부적으로 Java 보조 도구를 사용해 BCrypt 해시 생성과 SQL 실행을 처리합니다.

```text
scripts/BCryptHashTool.java  -> seed 비밀번호 BCrypt 해시 생성
scripts/JdbcSqlRunner.java   -> 생성된 SQL을 JDBC로 실행하고 table count 출력
```
...
| `SERVER_PORT` | 서버 포트 |
| `CUSTOM_ALLOWED_ORIGINS` | CORS 허용 Origin |
| `JWT_SECRET` | Base64 인코딩된 JWT 서명 secret |
| `JWT_ACCESS_TOKEN_EXPIRATION_MS` | access token 만료 시간 |
| `JWT_REFRESH_TOKEN_EXPIRATION_MS` | refresh token 만료 시간 |
| `SUPABASE_DB_URL` | Supabase PostgreSQL JDBC URL |
| `SUPABASE_DB_USERNAME` | Supabase DB username |
| `SUPABASE_DB_PASSWORD` | Supabase DB password |
| `SUPABASE_DDL_AUTO` | Hibernate ddl-auto 설정 |
| `FILE_UPLOAD_DIR` | 업로드 파일 저장 경로 |
| `SMS_API_KEY` | CoolSMS API key |
| `SMS_API_SECRET` | CoolSMS API secret |
| `SMS_SENDER` | 인증 문자 발신 번호 |

## Docs

- [API Routes](docs/api-routes.md)
- [User Controller Flow](docs/user-controller-flow.md)
- [Faker Seed Step](docs/step-01-faker-seed.md)
- [Troubleshooting](docs/troubleshooting.md)

## Implementation Decisions

### Why separate RefreshToken from User?

refresh token은 인증 세션에 가까운 데이터입니다.  
사용자 기본 정보와 생명주기가 다르기 때문에 `User` 엔티티 필드가 아니라 별도 `RefreshToken` 엔티티로 분리했습니다.

### Why store refresh token hash?

refresh token 원문이 DB에 저장되면 DB가 노출됐을 때 바로 재사용될 수 있습니다.  
그래서 BCrypt 비밀번호처럼 원문을 직접 저장하지 않고, SHA-256 해시를 저장한 뒤 요청으로 들어온 token을 다시 해시해 비교합니다.

### Why use LAZY and EntityGraph together?

모든 연관관계를 EAGER로 두면 필요하지 않은 API에서도 많은 조인이 발생합니다.  
반대로 LAZY만 사용하면 DTO 변환 시 N+1 문제가 생길 수 있습니다.

그래서 기본은 LAZY로 두고, 조회 응답에서 필요한 관계만 EntityGraph로 명시해 가져오는 방식을 사용했습니다.

### Why Scheduler instead of Batch?

현재 만료 경기 마감은 단순한 주기적 bulk update입니다.  
복잡한 재시도, chunk 처리, 대용량 단계 관리가 필요한 수준은 아니기 때문에 Spring Scheduler가 더 단순하고 적합합니다.

데이터 규모가 더 커지고, 실패 재처리나 실행 이력 관리가 중요해지면 Spring Batch로 고도화할 수 있습니다.

## Lessons Learned

- JWT 인증은 토큰 생성보다 Security Filter Chain과 권한 변환 흐름을 이해하는 것이 중요합니다.
- access token과 refresh token은 용도가 다르므로 decoder와 validator도 분리하는 편이 안전합니다.
- `@Transactional` 안에서 조회한 엔티티는 영속 상태이므로 setter 없이 도메인 메서드 변경만으로 dirty checking이 동작합니다.
- REST API 응답 형식은 Controller 예외뿐 아니라 Security 401/403까지 함께 맞춰야 클라이언트 처리가 단순해집니다.
- VO를 엔티티 필드로 사용하면 초반 설정은 늘어나지만, 값 검증 위치가 명확해집니다.
- FK, unique constraint, 서비스 검증을 함께 사용해야 매칭 도메인의 데이터 정합성을 지킬 수 있습니다.
